...
def add_numeric_column(df, length, label="Sig_id", dist=None, vals=None):
    ...
    if dist is None or dist == "range":
        ...
        start = time.time()
        df[label] = stats.norm.rvs(
            (vals[0] + vals[1]) // 2, abs(vals[0] - vals[1]) // 6, length
        )
        spt += time.time() - start
...
def add_timeseries_column(
    df, dataset_length, time_length, sig_type, params, label="Timeseries"
):
    ...
    if sig_type == "CAR":
        ...
        for i in tqdm(range(dataset_length)):
            ...
            if "starts" in params:
                mparams["start"] = (
                    params["starts"][i]
                    if len(params["starts"]) == dataset_length
                    else secrets.choice(params["starts"])
                )
            else:
                mparams["start"] = 0.01
            if "sectors" in params:
                ...
                mparams["growth"] = (
                    ...
                    else secrets.choice(params["growths"])
                )
        ...
        for i in tqdm(range(dataset_length)):
            mparams["sigma"] = params["sigma"] if "sigma" in params else 0.5
            mparams["ar_param"] = (
                params["ar_param"] if "ar_param" in params else [1.5, -0.75]
            )  # arbitrary numbers for AR(2) model
            mparams["start_value"] = (
                params["start_value"] if "start_value" in params else [None]
            )
        ...
        for i in tqdm(range(dataset_length)):
            ...
            mparams["coefficients"] = (
                params["coefficients"]
                if "coefficients" in params
                else [0.3, 0.05, 1.5, 0.1]
            )
            mparams["initial_condition"] = (
                params["initial_condition"] if "initial_condition" in params else None
            )
            mparams["error_initial_condition"] = (
                params["error_initial_condition"]
                if "error_initial_condition" in params
                else None
            )
        ...
        for i in tqdm(range(dataset_length)):
            if "tau" in params:
                mparams["tau"] = (
                    params["tau"][i]
                    if len(params["tau"]) == dataset_length
                    else secrets.choice(params["tau"])
                )
            else:
                mparams["tau"] = 16
        ...
        for i in tqdm(range(dataset_length)):
            if "frequency" in params:
                mparams["frequency"] = (
                    params["frequency"][i]
                    if len(params["frequency"]) == dataset_length
                    else secrets.choice(params["frequency"])
                )
            else:
            ...
            if "amplitude" in params:
                mparams["amplitude"] = (
                    params["amplitude"][i]
                    if len(params["amplitude"]) == dataset_length
                    else secrets.choice(params["amplitude"])
                )
            else:
            ...
            if "ftype" in params:
                mparams["ftype"] = (
                    params["ftype"][i]
                    if len(params["ftype"]) == dataset_length
                    else secrets.choice(params["ftype"])
                )
            else:
...
def add_noise(
    length, og_sig, red=False, white=False, rparams={}, wparams={}
    ...
    if white:
        ...
        red_noise = ts.noise.RedNoise(
            std=rstd, mean=rmean, tau=rtau, start_value=rstart_value
        )
        for i in tqdm(range(length)):
            t = time_vector[i]
...
def add_anomalies(length, num, amp):
    start = time.time()
    pos_impulses = signal.unit_impulse(
        length, random.sample(range(length), num)
    ) * random.sample(range(0, amp), 1)
    neg_impulses = signal.unit_impulse(
        length, random.sample(range(length), num)
    ) * random.sample(range(-1 * amp, 0), 1)
...
def gen_ar_signal(length, params):
    ...
    time_sampler = ts.TimeSampler(stop_time=length)
    regular_time_samples = time_sampler.sample_regular_time(num_points=length)
    ar_p = ts.signals.AutoRegressive(
        ar_param=params["ar_param"],
        sigma=params["sigma"],
...
def gen_narma_signal(length, params):
    ...
    narma = ts.signals.NARMA(
        ...
        error_initial_condition=params["error_initial_condition"],
        seed=params["seed"],
    )
...
def gen_mg_signal(length, params):
    npt = 0
    time_sampler = ts.TimeSampler(stop_time=length)
    irregular_time_samples_mg = time_sampler.sample_irregular_time(
        num_points=length * 2, keep_percentage=50
    )
    ...
    if params["equil"] != 1:
        start = time.time()
        mg_samples = mg_samples + np.linspace(
            params["equil"] - 1, params["equil"] - 1, length
        )
        npt += time.time() - start
    ...
    if "init_exp" in params:
        ...
        start = time.time()
        xlim = (
            np.where(mg_samples[50:] < ylim + 0.01)
            and np.where(mg_samples[50:] > ylim - 0.01)
        )[0][0] + 50
        root = np.sqrt(ylim)
...
def gen_pseudoperiodic_signal(length, params):
    npt = 0
    time_sampler = ts.TimeSampler(stop_time=length)
    irregular_time_samples_pp = time_sampler.sample_irregular_time(
        num_points=length * 2, keep_percentage=50
    )
